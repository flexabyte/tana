...
impl Scroller {
    ...
    pub fn new(url: String, index: String, slice_id: i64, max_slices: i64) -> Result<Scroller, HttpError> {
        ...
        Ok(Scroller{index: index, slice_id: slice_id, max_slices: max_slices, client: client, url: url})
    ...
    pub async fn scroll(self, creds: Credentials) -> Result<i64, HttpError> {
        ...
        let json_str = self.client.get(&url).body(payload)
            .header("Content-Type", "application/json")
            .basic_auth(&creds.username, 
            Some(&creds.password)).send().await?.text().await?;
        ...
        // Scroll to end
        total_hits_json = format!("{}{}", total_hits_json, self.scroll_to_end(&creds, scroll_id.clone()).await?);

        // Dump to disk
        println!("Writing {:?} documents.", total_hits_json.matches("\n").count()/2);
        Scroller::write_to_disk(format!("{}-{}.json", &self.index, &self.slice_id), total_hits_json);
    ...
    async fn scroll_to_end(&self, creds: &Credentials, mut scroll_id: String) -> Result<String, HttpError> {
        ...
        while hit_count != 0 {
            ...
            let json_str = self.client.post(&url).body(payload)
                .header("Content-Type", "application/json")
                .basic_auth(&creds.username, 
                Some(&creds.password)).send().await?.text().await?;
    ...
    fn parse_json(json_str: String) -> (String, i64, String) {
        ...
        for hit in hits_vec.iter() {
            let index = Index{_index: hit["_index"].as_str().unwrap_or("").to_string(),
            ...
            if action_meta_parsed.is_ok() && source_parsed.is_ok() {
                ...
                println!("Failed to parse action meta or source: {:?} {:?}", action_meta_parsed.err(), source_parsed.err());
            }
        }
    ...
    fn write_to_disk(filename: String, hits_json: String) {
        ...
        file.write(hits_json.as_bytes()).expect("Unable to write file");
    }
    ...
    async fn clear(self, creds: Credentials, scroll_id: String) -> Result<(),HttpError> {
        ...
        let _json_str = self.client.delete(&url).body(payload)
            .header("Content-Type", "application/json")
            .basic_auth(&creds.username, 
            Some(&creds.password)).send().await?.text().await?;
